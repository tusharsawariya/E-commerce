function SignInComponent(token) {
  ...
  signInForm.setAttribute(
    "style",
    "width:30%; height:300px; box-shadow: 0 0 5px grey"
  );
  ...
  headerDiv.setAttribute(
    "class",
    "d-flex bg-danger justify-content-center align-items-center"
  );
  ...
  signInButton.addEventListener("click", function () {
    signInAction(emailInput, passwordInput);
...
function signInAction(emailInput, passwordInput ) {
  ...
  //   : window.alert("Invalid email id and password");
...
function DashBoardComponent(user ,token) {
  var mainDiv = document.getElementById("main");

  mainDiv.innerHTML = "";
  //  mainDiv.style.backgroundColor='black';
  let header = document.createElement("div");
  header.setAttribute("id", "top-head");
  mainDiv.appendChild(header);
  let cardRender = document.createElement("div");
  cardRender.setAttribute("id", "Data-head");
  //  cardRender.style.backgroundColor='gray';
  mainDiv.appendChild(cardRender);

  HeaderComponent(mainDiv, user ,token);
  CardComponent(JSON.parse(localStorage.getItem("product-list")));

  //  Event listner for handling events
  document.querySelector("#card-container").addEventListener(
    "click",
    function (e) {
      // console.log("first invokee ");
      // console.log(e.target.value);
      // console.log("THis is rahul")
      CehckProduct(e.target.value);
      // return function() {
...
function HeaderComponent(mainDiv, user ,token) {
  ...
  headerDiv.setAttribute("class", "d-flex"); //bg-dark

  var logoDiv = document.createElement("div");

  //  logoDiv.setAttribute("style","width: 20%; height: 60px; "); //border:1px solid red;
  //  loggoDiv;
  //  logoDiv.innerHTML="<img id="logoimg">";
  logoDiv.setAttribute(
    "style",
    "width: 20%; height: 60px; display: flex; justify-content: center;" // align-items: center;
  );

  let limg = document.createElement("img");

  limg.setAttribute("src", "./Img/logo.png");
  limg.setAttribute("style", "height: 100%; width:65%;"); //border: 2px solid black;
...
document.querySelector("#card-container").addEventListener(
  ...
  searchInputDiv.setAttribute(
    "style",
    "width: 60%;height: 60px; display:flex; padding: 12px;"
  ); //border:1px solid red;
  searchInputDiv.setAttribute("class", "justify-content-between");
  // searchInputDiv.style.Color = "yellow";
  // let searchBar = document.createElement("div");

  let searchinput = document.createElement("input");
  searchinput.setAttribute("class", "form-control mr-sm-2");
  searchinput.style.backgroundColor = "rgb(208, 206, 205 )";
  searchinput.setAttribute("type", "search");
  searchinput.setAttribute("placeholder", "search");
  searchinput.setAttribute("aria-label", "Search");
  // searchinput.placeholder = "Search Product here";
  // searchinput.setAttribute("style", "padding: 5px; width:100%; height: 60px;");
  // let cehckpoiinnt =0;
  ...
  // =============Modifying this search 
  searchinput.addEventListener("input",function (){
    let timer;
    return function (event) {
      clearTimeout(timer);
      timer = setTimeout(() => searchProduct(event), 1000);
    };
  }());

   //searching products here
  //  searchinput.setAttribute('placeholder', "Searc here ");
  ...
  menuDiv.setAttribute(
    "style",
    "width: 20%; height: 60px; display: flex;justify-content:space-evenly; padding:12px;"
  ); //border:1px solid red;
  //

  let addtocart = document.createElement("i");
  // addProduct.innerText = "Add Product";
  // addProduct.style.width = "100%";
  // addProduct.style.border = "none";
  addtocart.setAttribute("class", "fa-solid fa-cart-shopping");
  addtocart.setAttribute("style", "font-size:40px;");

  addtocart.addEventListener("click", viewCartItem); // addd to cart implimentation panding

  let signout = document.createElement("button");
  signout.setAttribute("class", "btn btn-outline-dark");
  ...
  document.getElementById("top-head").appendChild(headerDiv);
}

function viewCartItem() {
  // view cartt item ==============================
  if(!token)window.alert("Please login first ...");
   else{  
  var currentUser = JSON.parse(localStorage.getItem("current-user"));
  // console.log(  "coming directy ",user);
  // console.log(  "coming here ",currentUser);
  // console.log(  "token ",token);
  var cartList = JSON.parse(localStorage.getItem("cart-list"));
  if (!cartList) {
    cartList = [];
    console.log("no product add yet ....");
    window.alert("empty card");
  }
  var index = cartList.findIndex((cartItem) => {
    return cartItem.email == currentUser.email;
  });

  if (index != -1) {
    // second time user performing add to cart
    var cart = cartList[index];
    // console.log("Inside the add cart This area the product :",cart);
    // Write check for item already exist or not
    // var p = cart.cartItems.find((item)=>{return item.id == product.id});
    // window.alert("products available ");
    // console.log("Product item :",cart.cartItems);
    // console.log(cart.cartItems.length);
    viewCartItemUI( cartList,cart.cartItems ,index);
...
// ====================================================================================================
//view cart products
function viewCartItemUI( cartList, cart_cartItems ,currentUserindex) {

  console.log("VIewCard Item UI : ",cart_cartItems);
  console.log("VIewCard Item UI lenght : : ",cart_cartItems.length);
  let Data_head = document.getElementById("card-container");  //document.getElementById("card-container");
  Data_head.innerHTML = "";
  let viewcardmain = document.createElement("div");

  viewcardmain.setAttribute("class", "container");
  viewcardmain.setAttribute(
    "style",
    "width:100%; border: 2px solid black;  padding: 5%;"//height; 500px;
  );

  let h2 = document.createElement("h2");
  h2.innerText = "Your Items";
  viewcardmain.appendChild(h2);
  // h2.setAttribute("style", "margin:50px;")
  let h3 =document.createElement('h3');

  if (cart_cartItems.length < 1) {
    h3.innerText = "Your cart is empty..";
    viewcardmain.appendChild(h3);
  } 
  else {
    h3.innerText = `You have added ${cart_cartItems.length} products`;
...
for (let product of cart_cartItems) {
    let itemdiv = document.createElement('div');
    itemdiv.setAttribute("style", "width:100%; height:20%; border: 2px solid black; display: flex; padding: 15px; margin-bottom: 15px;");

    let img = document.createElement('div');
    img.setAttribute("style", "height:auto; width: 250px; border: 2px solid black; margin: 5px; margin-right: 50px;");

    let CartImg = document.createElement('img');
    CartImg.setAttribute("src", `${product.thumbnail}`);
    CartImg.setAttribute("style", "height:100%; width:100%; border:2px solid red; margin: 4px;");
    ...
    btn_view.setAttribute("value",`${product.id}`)
    btn_view.addEventListener("click", (e)=> {
let index =e.target.value;
      // view product 
      CehckProduct(index);
      console.log( "index click value ",index)

    })

let removecart =document.createElement('button');
removecart.innerText='remove';
removecart.setAttribute("id", `${product.id}`);
removecart.addEventListener("click", (e)=>{
console.log( "Product id :",e.target.id)

let UpdateItems =cart_cartItems.filter( (product)=>{

  return product.id !=e.target.id;
});

console.log("remoable itme : ", UpdateItems.length)

  cartList[currentUserindex].cartItems=UpdateItems;

  localStorage.setItem("cart-list", JSON.stringify(cartList));
  console.log("update sussesfully", cartList);
  viewCartItem();
    ...
    ProductDAta.appendChild(h3);
    ProductDAta.appendChild(BookItem);
    ProductDAta.appendChild(btn_view);
    ProductDAta.appendChild(removecart);

    itemdiv.appendChild(img);
    itemdiv.appendChild(ProductDAta);
    viewcardmain.appendChild(itemdiv);
    ...
    // let cnt=
    // document.querySelector(".cnt-1")
    // .addEventListener("input", () => {console.log("kknaN")})
    total.setAttribute("style", "height:50px; width: 100%; border: 2px solid red;");
    let totalprice =document.createElement('h4');
    // totalprice.innerHTML =`Total Amount : ${}`;
let PlaceOrder= document.createElement('button');

PlaceOrder.innerText="Place Order";
PlaceOrder.setAttribute("class","btn btn-success");
PlaceOrder.setAttribute("type","button");


total.appendChild(totalprice);
total.appendChild(PlaceOrder);
viewcardmain.appendChild(total);


  }

  Data_head.appendChild(viewcardmain);
}



// this will find the search Item
let searchProduct = (input) => {
  let temparray = [];
  if (input.target.value) {
    ...
    if (temparray.length > 0) {
      // temparray.push(data);
      console.log("temp array lenth  : ", temparray.length);
      var mainDiv = document.getElementById("Data-head");

      mainDiv.innerHTML = "";

      CardComponent(temparray);
    ...
    // data ? CardComponent(temparray) :
  } else {
    let cardBar = document.getElementById("Data-head");
    cardBar.innerHTML = "";
    CardComponent(JSON.parse(localStorage.getItem("product-list")));
    //event lisner
    document.querySelector("#card-container").addEventListener(
...
// let searching = handaleSearch(searchProduct, 1000);

function CardComponent(productList) {
  var mainDiv = document.querySelector("#Data-head");
  ...
  for (let product of productList) {
    ...
    td.setAttribute("id", `${product.id}`);
    td.setAttribute("class", "product-data");
    td.setAttribute("style", "width: 100%;  height: 100%; "); //border : 2px solid black;
    // td.innerText=` Product Id : product${cnt}`
    cardCol.appendChild(td);

    renderProduct(product, td);
    // cnt++;

    // slider code for image rendering
  }
...
//   Product book or add to card

let CehckProduct = (index) => {
  // here showing full product data and two button Add to card And Book ;
  let cardComponent = document.getElementById("card-container");
  // console.log("Check point Index : ", index);
  ...
  if (index) {
    ...
    // for(let product of productList)
    let product = productList[index-1];
    // console.log("item: ", product);

    let MainItem = document.createElement("div");
    MainItem.setAttribute("class", "MainItem");
    MainItem.setAttribute("style", "width:100%;  display:flex;");

    // MainItem.style.backgroundColor = 'red';

    let ChilldItem = document.createElement("div");
    ChilldItem.setAttribute(
      "style",
      "width: 480px; height:450px; margin-top:10%;margin-left:7%; padding: 20px;"
    );

    let sliderMain = document.createElement("div");
    sliderMain.setAttribute("id", "carouselExampleIndicators");
    sliderMain.setAttribute("class", "carousel slide");
    sliderMain.setAttribute("data-ride", "carousel");
    // sliderMain.setAttribute("style", "border:2px solid ; border-radius: 11px;");
    ...
    ol.appendChild(li2);
    ol.appendChild(li3);
    sliderMain.appendChild(ol);

    let carouselInner = document.createElement("div");
    carouselInner.setAttribute("class", "carousel-inner");

    let createCarouselItem = (src, isActive) => {
      let carouselItem = document.createElement("div");
      carouselItem.setAttribute(
        "class",
        `carousel-item ${isActive ? "active" : ""}`
      );
      let courImg = document.createElement("img");
      courImg.setAttribute("class", "d-block w-100");
      courImg.setAttribute("src", src);
      courImg.setAttribute("alt", "Slide");
      courImg.setAttribute("style", "height:100%; width:100%;border-radius: 21px; ");
      carouselItem.appendChild(courImg);
      return carouselItem;
    };

    carouselInner.appendChild(createCarouselItem(product.images[0], true));
    carouselInner.appendChild(createCarouselItem(product.images[1], false));
    carouselInner.appendChild(createCarouselItem(product.images[2], false));

    sliderMain.appendChild(carouselInner);

    let createControl = (direction, href, spanClass, text) => {
      let control = document.createElement("a");
      control.setAttribute("class", `carousel-control-${direction}`);
      control.setAttribute("href", href);
      control.setAttribute("role", "button");
      control.setAttribute("data-slide", direction);

      let span1 = document.createElement("span");
      span1.setAttribute("class", spanClass);
      ...
      control.appendChild(span1);
      control.appendChild(span2);
      return control;
    };

    sliderMain.appendChild(
      createControl(
        "prev",
    ...
    ChilldItem.appendChild(sliderMain);

    let dataContainer = document.createElement("div");

    dataContainer.setAttribute(
      "style",
      "width: 40%; height:40%;   margin:2%; margin-top:10%; padding: 25px; font-size; 35px"
    );

    dataContainer.innerHTML = `<h2> ${product.title} </h2> 
<h6>Brand  :  ${product.brand} </h6> 
<h6 style="color:red">Price  :  ${product.price} </h6> 
<h6>Discounted   :  ${product.discountPercentage}% </h6> 
...
`;
    ...
    btn1.setAttribute('style',"margin-right:25px ;border-radius:11px")
    ...
    btn2.setAttribute('style',"border-radius:11px")

    dataContainer.appendChild(btn1);
    dataContainer.appendChild(btn2);
    MainItem.appendChild(ChilldItem);
    MainItem.appendChild(dataContainer);
    cardComponent.appendChild(MainItem);
  }
};

// =======================================HANDAL ADD TO CART ==========================
function handleAddToCartAction(product) {
  // console.log("not  true: ",!token ,  "  token is true ; ", token)
  if(!token)window.alert("Please login First ....");
  else{ 
  var currentUser = JSON.parse(localStorage.getItem("current-user"));
  var cartList = JSON.parse(localStorage.getItem("cart-list"));
  ...
  if (index != -1) {
    ...
    var cart = cartList[index];
    // Write check for item already exist or not
    var p = cart.cartItems.find((item) => {
...
function noRecord() {
  let cardBar = document.getElementById("Data-head");
  cardBar.innerHTML = "";
  cardBar.innerHTML = "<h1> No Record Found .....</h1>";
}

function renderProduct(product, td) {
  let Pimg = document.createElement("div"); //first div for imgs
  Pimg.setAttribute(
    "style",
    "height: 55%; width: 100%;  margin: 1px;  align-content: center; "
  ); //border: 1px solid black;
  // Pimg.innerText='product img';
  Pimg.setAttribute("class", "Pimg");
  // Pimg.setAttribute("value",`${cnt}`);
  Pimg.value = `${product.id}`;
  let img = document.createElement("img");
  img.setAttribute("src", `${product.images[0]}`);
  img.setAttribute("style", "width: 100%; height: 100%; padding: 15px;");
  // img.setAttribute("value",`${cnt}`);
  img.value = `${product.id}`;
  ...
  productdata.setAttribute(
    "style",
    "width: 100%; height:calc(100% - 55%);  margin: 1px; padding: 15px; font-size: 10px;"
  ); //border: 2px solid brown;
  productdata.setAttribute("class", "P-data");
  // productdata.setAttribute("value",`);
  productdata.value = `${product.id}`;
  // productdata.style.backgroundColor='red';
  productdata.innerHTML = `<h6 class="p-title" > ${product.title} </h6> <p class="b-brand">Brand :${product.brand} <br>
